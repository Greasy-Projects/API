import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, unique, varchar, datetime, text, mysqlEnum } from "drizzle-orm/mysql-core"
...
export const users = mysqlTable("users", {
	id: varchar("id", { length: 100 }).notNull(),
	primaryAccountId: varchar("primary_account_id", { length: 100 }).notNull().references((): AnyMySqlColumn => userAccounts.id, { onDelete: "cascade", onUpdate: "restrict" } ),
...
(table) => {
	return {
		usersPrimaryAccountIdUnique: unique("users_primary_account_id_unique").on(table.primaryAccountId),
	}
});
...
export const userAccounts = mysqlTable("user_accounts", {
	...
	userId: varchar("user_id", { length: 100 }).default('NULL').references((): AnyMySqlColumn => users.id, { onDelete: "cascade", onUpdate: "restrict" } ),
